...
send_initial_msg() {
    RESPONSE=$(curl -s -X POST "https://api.telegram.org/bot$TG_TOKEN/sendMessage" \
        ...
        --data-urlencode text="🚀 <b>Build Started: ProjectSakura for fog/wind/rain</b>" \
        -d parse_mode="HTML")
...
git clone https://github.com/AloozChips/device_xiaomi_fog.git device/xiaomi/fog -b sakura --depth 1
...
ZIP=$(find "$ROM_DIR" -maxdepth 1 -type f -iname "*sakura*.zip" -printf '%T@ %p\n' | sort -n | tail -1 | cut -f2- -d" ")
...
🛡️ <b>SHA256:</b> <code>$SHA256_HASH</code>"
    ...
    edit_msg "❌ <b>Build Failed: ProjectSakura for fog/wind/rain.</b>
